...
#define MAX_BL	255
/*
 * 24 -> MIDNIGHT: added one extra step in aries-panel.c for
 * real low manual brightness @ 0%.
 * If we add more than one step more (lower end) autobrightness
 * uses lower settings, too. 
 */
...
static void update_brightness(struct s5p_lcd *lcd)
{
	...
	update_acl(lcd);
#endif
	/*if (lcd->on_19gamma) // SPEEDMOD disabling gamma 1.9 ported for Midnight
